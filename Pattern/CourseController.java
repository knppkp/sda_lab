...
public class CourseController extends JPanel implements Observer, ChangeListener, ActionListener {
    ...
    public void update(Observable o, Object arg) {
        if (arg instanceof ArrayList<?>) {
            ArrayList<?> rawList = (ArrayList<?>) arg;

            // Optional: check element types for safety
            if (!rawList.isEmpty() && rawList.get(0) instanceof CourseRecord) {
                @SuppressWarnings("unchecked")
                ArrayList<CourseRecord> newCourses = (ArrayList<CourseRecord>) rawList;

                // ✅ Now you can safely use newCourses
                for (int i = sliders.size(); i < newCourses.size(); i++) {
                    this.addCourse(newCourses.get(i));
                }
            }
        }
    }
    ...
    /**
     * Manages the creation of a new course. Called when the "New Course" button
     * is pressed.
     *
     * @param arg0 not used
     */
    ...
    public static void main(String[] args) {
        ...
        data.notifyObservers(data.getUpdate());
